...
/* =======================================================================
 * jQuery.Huisidenav.js PC左侧菜单-隐藏显示
 * ======================================================================== */
function displaynavbar(obj){
	if($(obj).hasClass("open")){
		$('#switchAside').html('<i class="fa fa-dedent"></i>');
		$(obj).removeClass("open");
		$("body").removeClass("big-page");
	} else {
        $('#switchAside').html('<i class="fa fa-indent"></i>');
		$(obj).addClass("open");
...
/* =======================================================================
 * jQuery.Huisidenav.js Mob端左侧菜单-隐藏显示
 * ======================================================================== */
function mobdisplaynavbar(obj){
    if($(obj).hasClass("open")){
        $(obj).removeClass("open");
        ...
        $(obj).addClass("open");
        $('.Hui-article-box').stop().animate({'left':'305px'},150);
        $('aside').stop().animate({'left':'0px'},150);
        $('.Hui-article-box').addClass('ngFixed');
        $('.Hui-article').addClass('ngBlur');
        $('.Hui-article-box').append('<div class="ngMobMask"></div>');
        $('.ngMobMask').fadeIn();
    }
}
			...
			that.find(options.titCell).on(options.trigger,function(){
				if ($(this).next().is(":visible")) {
					if (options.type == 2) {
                        ...
                        $(this).next().find('a').stop().animate({'padding-left':'0px','opacity':'0'},options.speed);
						$(this).next().fadeOut(options.speed).end().removeClass(options.className);
						if ($(this).find('span')) {
							$(this).find('span').html('<i class="fa fa-angle-right"></i>');
						}
					}
				}else {
					if (options.type == 3) {
						$(this).next().find('a').stop().animate({'padding-left':'55px','opacity':'1'},options.speed);
						$(this).next().fadeIn(options.speed).end().addClass(options.className);
						if ($(this).find('span')) {
							$(this).find('span').html('<i class="fa fa-angle-down"></i>');
						}
					} else {
                        $(this).next().find('a').stop().animate({'padding-left':'0px','opacity':'0'},options.speed);
						that.find(options.mainCell).fadeOut(options.speed);
						that.find(options.titCell).removeClass(options.className);
						if (that.find(options.titCell).find('span')) {
							that.find(options.titCell).find('span').html('<i class="fa fa-angle-right"></i>');
						}
                        $(this).next().find('a').stop().animate({'padding-left':'55px','opacity':'1'},options.speed);
						$(this).next().fadeIn(options.speed).end().addClass(options.className);
						if ($(this).find('span')) {
